...
- **Controls all Fritz!DECT and Fritz!Smart Home device types:**
  ...
  | Battery-powered devices | Real-time battery level (%) with color-coded status indicator and low-battery warning; devices that support both battery and external (USB/mains) power (e.g. FRITZ!Smart Energy 250) are shown with their actual, live-detected power source instead of a misleading battery level |
  ...
  | **Local groups** | User-defined groups that combine any mix of Fritz!Box devices **and Fritz!Box hardware groups** into a single tree entry — managed via **Tools → Manage Local Groups…**; support the same stacked energy history, stacked power chart, multi-line temperature chart, and per-member pie chart as Fritz!Box hardware groups; nested local groups are supported with full cycle detection and deduplication |

- **Power-role classification** — individual energy-capable devices can be assigned one of two mutually exclusive roles, each with its own checkbox that appears right-aligned in the chart dock header and in a right-click context menu on the device tree (also shown inline in the device control panel):
  - **Power producer** (`isProducer`) — values are negated throughout: power and energy charts show production as negative values, the tree view negates the displayed wattage, and group charts separate consumer bars (stacking upward) from producer bars (stacking downward). The energy gauge shows "Total Energy Produced". Group views overlay a **net effective power/energy indicator**: a black line on the rolling power chart and a semi-transparent ghost bar on the energy history chart, showing the signed sum (consumption minus production).
  - **Native net power meter** (`nativeNetPower`) — the device already reports signed net power (positive = consuming, negative = producing); values are used as-is with no sign inversion. The energy gauge shows "Net Energy" for these devices. In group views, the net overlay is also shown whenever any member has this flag set.
...
- **Live charts** (Temperature, Power, Humidity, **Battery status**, Energy gauge, Energy history with configurable resolution; bar chart shows hover tooltips with date/value, and the most recent bar is visually dimmed to indicate an incomplete accumulation period; group devices show a **stacked energy history bar chart** with one colour-coded bar segment per member device, a **stacked per-member power chart**, and a **multi-line temperature chart** with one line per temperature-capable member). Hovering over the **Power** chart shows a vertical dotted crosshair line and a persistent tooltip (time + per-member/net power values) that stays visible until the mouse leaves the chart, rather than auto-hiding on a timer. For groups the Energy tab also shows a **per-member pie chart** summarising each member's share of total energy; slice labels show both absolute and percentage values, with overlap-aware visibility that hides labels of small neighbouring slices; hovering a slice explodes it and shows a tooltip with its details.
  - **Battery Status** tab (for battery-powered devices): displays real-time battery level percentage with color-coded indicator (green > 50%, orange 10-50%, red < 10%, gray N/A), large battery icon, human-readable status text (e.g., "Good", "Low", "Critical"), visual progress bar, and red warning banner when Fritz!Box flags low battery. For dual-powered devices (battery + USB/mains), a green banner replaces the low-battery warning whenever the Fritz!Box confirms the device is currently running on external power, so a stale/unused battery level is never mistaken for the active power source; the same detection also suppresses the misleading battery overlay icon in the device tree.
